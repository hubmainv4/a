...
if not hookmetamethod then localPlayer:Kick("Executor not supported, use a better executor. dm vasli for recommendations") end

local realV, fakeV = rootPart.Velocity, Vector3.new()

local oldI; oldI = hookmetamethod(game, "__index", function(self, key)
    if self == rootPart and key == "Velocity" then return fakeV end
    return oldI(self, key)
end)

rootPart:GetPropertyChangedSignal("Velocity"):Connect(function() realV = rootPart.Velocity end)
localPlayer.CharacterAdded:Connect(function(c)
    rootPart = c:WaitForChild("HumanoidRootPart")
    realV = rootPart.Velocity
    rootPart:GetPropertyChangedSignal("Velocity"):Connect(function() realV = rootPart.Velocity end)
end)
